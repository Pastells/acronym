...
try:
    ...
    for d in ["words", "brown", "gutenberg", "cess_cat", "cess_esp"]:
        nltk.download(d)
import argparse
import textwrap
import sys
...
def _index_in(s, word, offset=0, must_start=False):
    ...
    # recursively search interior of substring
        ...
        sub_s = s[start + 1 : end]
        sub_word = word[1:-1]
        sub_result = _index_in(
            sub_s, sub_word, offset=start + offset + 1, must_start=False
        )
...
def find_acronyms(s, corpus, min_length=5, max_length=7):
    ...
    short_list = np.unique(
        [w.lower() for w in full_list if 3 <= len(w) and w.isalpha()]
    )
    word_list = np.unique(
        [
            w.lower()
    ...
    results = pd.DataFrame({"long_version": results, "score": scores})
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(textwrap.dedent("Scoring System:\n" + score_acronym.__doc__)),
    )
    parser.add_argument(
        "name",
        metavar="ProjectName",
    ...
    parser.add_argument(
        "--min-length", default=4, type=int, help="minimum length acronym to generate"
    )
    parser.add_argument(
        "--max-length", default=9, type=int, help="maximum length acronym to generate"
    )
    parser.add_argument(
        "--max-results",
        default=30,
    ...
    parser.add_argument(
        ...
        type=str,
        default="english",
        help="Choose between 'catalan', 'spanish' or 'english'",
    )
    parser.add_argument(
    ...
    elif args.language == "catalan":
        corpus = nltk.corpus.cess_cat
    elif args.language == "spanish":
        corpus = nltk.corpus.cess_esp
    else:
        raise ValueError("Wrong language, must be english, catalan or spanish")

    results = find_acronyms(
        args.name, corpus, min_length=args.min_length, max_length=args.max_length
    )
    ...
    for i, (key, row) in enumerate(
        results.sort_values("score", ascending=False).iterrows()
    ):
